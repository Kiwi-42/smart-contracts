...
contract('test', async accounts => {
  it('sandwich attacks', async () => {
    const deployer = accounts[0];
    const attacker = accounts[1];
    const victim = accounts[2];

    const ampBps = new BN(50000); // 5
    ...
    await token0.transfer(pool.address, token0Amount.div(new BN(100)), {from: victim}),
    await token1.transfer(pool.address, token1Amount.div(new BN(100)), {from: victim}),
    ...
    await token0.transfer(pool.address, token0Amount.div(new BN(100)).mul(new BN(99)), {from: victim}),
    await token1.transfer(pool.address, token1Amount.div(new BN(100)).mul(new BN(99)), {from: victim}),
    await expectRevert(pool.mint(victim, {from: victim}), 'DMMPool: OUT_OF_BOUNDS_VRESERVE');
    // uncomment the following line and comment the above line to see what happens after this point.
    // await pool.mint(victim, {from: victim});
